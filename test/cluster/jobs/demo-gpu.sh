...
# devices via qstat and publishes them as the vendor device variable:
# CUDA_VISIBLE_DEVICES by default, ROCR_VISIBLE_DEVICES under gpu.vendor: amd.
# Exactly one is ever written; the other is removed from the rank environment.
#
...
srun -n 2 bash -c 'echo "  rank $SLURM_PROCID on $(hostname): CUDA_VISIBLE_DEVICES=[${CUDA_VISIBLE_DEVICES-unset}] ROCR_VISIBLE_DEVICES=[${ROCR_VISIBLE_DEVICES-unset}]"'
echo "[--gpus-per-task=1: one device per rank]"
srun -n 2 --gpus-per-task=1 bash -c 'echo "  rank $SLURM_PROCID on $(hostname): CUDA_VISIBLE_DEVICES=[${CUDA_VISIBLE_DEVICES-unset}] ROCR_VISIBLE_DEVICES=[${ROCR_VISIBLE_DEVICES-unset}]"'
